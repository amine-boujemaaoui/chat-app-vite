...
async function socketHandler(socket) {
  ...
  socket.on("friendRequest", async ({ token, friendId }) => {
    try {
      ...
      if (friendSocketId) {
        ...
        console.log(`L'utilisateur ${friendId} n'est pas connecté.`);
      }
    } catch (error) {
  ...
  socket.on("friendRequestAccepted", async ({ token, friendId }) => {
    try {
      ...
      const [userFriendsList] = await db.query(friendsQuery, [userId]);
      socket.emit("friendsList", userFriendsList);

      const friendSocketId = connectedUsers[friendId];
      if (friendSocketId) {
        const [friendFriendsList] = await db.query(friendsQuery, [friendId]);
        io.to(friendSocketId).emit("friendsList", friendFriendsList);

        io.to(friendSocketId).emit("friendAccepted", { friendId: userId });
      }

      console.log(`Amitié créée entre ${userId} et ${friendId}`);
    } catch (error) {
